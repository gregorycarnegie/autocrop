...
fn reshape_buffer_to_image<'py>(
    ...
    if bytes_slice
        .as_ptr()
        .align_offset(std::mem::align_of::<u8>())
        == 0
    {
        // Create shape tuple
...
pub fn bgr_to_gray(image_view: ArrayView3<u8>, use_rec709: bool) -> Array2<u8> {
...
fn calc_alpha_beta(hist: &[f64]) -> Option<(f64, f64)> {
    if hist.iter().all(|&x| x == 0.0) || hist.is_empty() {
        // Check for empty or all-zero hist early
    ...
    let mut max_gray = accumulator
        .iter()
        .position(|&x| x > max_limit)
        .unwrap_or(hist.len() - 1);

    if max_gray == 0 && hist.len() > 1 {
        // Handle edge case where max_limit is very high
        max_gray = hist.len() - 1; // Use the last bin index
    }
    if max_gray == 0 {
        max_gray = 1
    } // Prevent division by zero if only one bin exists
...
pub fn convert_scale_abs(image_view: ArrayView3<u8>, alpha: f64, beta: f64) -> Array3<u8> {
...
fn get_rotation_matrix<'py>(
    ...
    let (left_mean, right_mean) = rayon::join(
        || {
            ...
            left_eye_ndarray.mean_axis(Axis(0)).ok_or_else(|| {
                PyRuntimeError::new_err("Failed to calculate mean axis for left eye")
            })
        },
        || {
            ...
            right_eye_ndarray.mean_axis(Axis(0)).ok_or_else(|| {
                PyRuntimeError::new_err("Failed to calculate mean axis for right eye")
            })
        },
    );
    ...
    // Apply scaling
    let center_x = if scale_factor > 1.0 {
        center_x_unscaled * scale_factor
    } else {
        center_x_unscaled
    };
    let center_y = if scale_factor > 1.0 {
        center_y_unscaled * scale_factor
    } else {
        center_y_unscaled
    };
...
fn compute_edges(
    ...
    let calc_edge =
        |p: f64, dim: f64, pad: u32| (p + dim * (0.5 + (pad as f64) / 100.0)).round() as i32;
...
fn crop_positions(
    ...
    let cropped_dimensions = compute_cropped_lengths(&face, dimensions, face_percent);

    Some(compute_edges(&face, cropped_dimensions, padding))
}
